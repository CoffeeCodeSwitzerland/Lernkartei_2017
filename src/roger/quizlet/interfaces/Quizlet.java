package roger.quizlet.interfaces;
...
public class Quizlet {
	...
	public ArrayList<String> SearchSet(String query) throws MalformedURLException, IOException {
		...
		String tempString = obj.getInt("total_results") + separator + obj.getInt("total_pages") + separator
				+ obj.getInt("image_set_count") + separator + obj.getInt("page");

		tempList.add(tempString);

		// Fügt die Sets dem Array hinzu
		for (int i = 0; i < arr.length(); i++) {
			tempString = Integer.toString(arr.getJSONObject(i).getInt("id")) + separator
					+ arr.getJSONObject(i).getString("title") + separator + arr.getJSONObject(i).getString("created_by")
					+ separator + arr.getJSONObject(i).getInt("term_count") + separator
					+ arr.getJSONObject(i).getBoolean("has_images") + separator
					+ arr.getJSONObject(i).getString("description") + separator
					+ arr.getJSONObject(i).getString("lang_terms") + separator
					+ arr.getJSONObject(i).getString("lang_definitions") + separator + i;
			tempList.add(tempString);
		}
	...
	public ArrayList<String> GetSet(String id) throws MalformedURLException, IOException {
		...
		String tempString = obj.getInt("id") + separator + obj.getString("title") + separator
				+ obj.getString("created_by") + separator + obj.getInt("term_count") + separator
				+ obj.getBoolean("has_images") + separator + obj.getString("description") + separator
				+ obj.getString("lang_terms") + separator + obj.getString("lang_definitions");

		tempList.add(tempString);

		// Karten
		for (int i = 0; i < arr.length(); i++) {
			tempString = arr.getJSONObject(i).getInt("id") + separator + arr.getJSONObject(i).getString("term")
